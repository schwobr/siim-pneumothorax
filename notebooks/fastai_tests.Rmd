...
from fastai.train import GradientClipping, AccumulateScheduler
...
from fastai.vision.image import Image, ImageSegment, image2np, pil2tensor, TfmLighting
...
os.environ['NEPTUNE_API_TOKEN']="eyJhcGlfYWRkcmVzcyI6Imh0dHBzOi8vdWkubmVwdHVuZS5tbCIsImFwaV9rZXkiOiJjOTJkZmE2Yy03MDQzLTQwN2QtYTU1Yy0yNzBiZDY3MTEyODkifQ=="
os.environ['NEPTUNE_PROJECT']="schwobr/SIIM-Pneumothorax"
os.environ['NEPTUNE_NOTEBOOK_ID']="d3f06d13-4d0d-4185-9fc0-4178915bdb35"
```
...
```{python hidden=TRUE, Collapsed=false}
def load_data_softmax(path, bs=8, train_size=256, sample_pct=None, xtra_tfms=None, seed=42, **db_kwargs):
    train_list = (PneumoSegmentationList.
                  from_csv(path.parent, path.name))
    if sample_pct is not None:
        train_list = train_list.use_partial_data(sample_pct=sample_pct, seed=seed)
    train_list = (train_list.
                 ...
                 transform(get_transforms(do_flip=False, xtra_tfms=xtra_tfms), size=train_size, tfm_y=True).
...
```{python hidden=TRUE, Collapsed=false}
def load_data_mtl(path, bs=8, train_size=256, xtra_tfms=None, **kwargs):
...
def replace_bn(learner, m, G=32):
    for n, ch in m.named_children():
        if isinstance(ch, nn.BatchNorm2d):
            new_layer = to_device(nn.GroupNorm(G, ch.num_features, ch.eps,  ch.affine), learner.data.device)
...
```{python Collapsed=false}
def _gaussian_noise(x, mean=0, min_std=5e-3, max_std=0.05):
    c, h, w = x.shape
...
def mtl_scores(learner, thrs, ds_type=DatasetType.Valid):
    ...
    for x, targs in tqdm(dl):
        ...
        y_cat = nn.Softmax(dim=1)(y_cat.to(learner.data.device))
        y_mask = nn.Softmax(dim=1)(y_mask.to(learner.data.device))
        n = y_mask.shape[0]
...
<!-- #region {"heading_collapsed": true, "Collapsed": "false"} -->
### Frozen training
<!-- #endregion -->

```{python hidden=TRUE, Collapsed=false}
db = load_data_softmax(LABELS, bs=BATCH_SIZE, train_size=TRAIN_SIZE, sample_pct=0.3, seed=SEED, xtra_tfms=[gaussian_noise()])
```
...
```{python hidden=TRUE, Collapsed=false}
clf_name = f'backbone_clf_{MODEL}_{TRAIN_SIZE}'
...
```{python hidden=TRUE, Collapsed=false}
learner = unet_learner(db, models[MODEL], pretrained=PRETRAINED,
                       loss_func=URLoss(nn.CrossEntropyLoss(), reduction='sum', do_spatial_reduc=True),
                       opt_func=RangerW,
                       wd=WD, model_dir=MODELS_PATH, metrics=[dice])
```
...
learner.fit_one_cycle(
    ...
    callbacks=[
        ...
        AccumulateScheduler(learner, min(8, 64 // BATCH_SIZE)),
        LookAhead(learner, k=max(6, 64//BATCH_SIZE)),
...
```{python Collapsed=false}
db = load_data_mtl(LABELS, bs=BATCH_SIZE, train_size=TRAIN_SIZE, device=torch.device('cpu'))
```
...
```{python Collapsed=false}
prev_save_name = f'uf_mtl_{MODEL}_{512}'
prev_save_name = f'{prev_save_name}_{getNextFilePath(MODELS_PATH, prev_save_name)-3}'
#prev_save_name = 'fold1_mtl_resnet34_256_2'
prev_save_name
```
